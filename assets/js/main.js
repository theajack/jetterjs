...
var _code={
  _str:1,
  _key:["function","if","else","var","return","for","true","false","switch","break","case","in","this"],
  _tag:3,
  _attr:4,
  //_sign:["{","}","&lt;","&gt;","\\[","\\]",",",";","\\+","-","=","\\(","\\)","%","|","\\/","&"],
  _sign:["=","{","}","\\(","\\)","\\[","\\]",",","&&","\\.","\\?","\\|","\\+","-",";\n",":","\\*"],//转义
...
/*function moveCursor(){
  var p = cursor.getPositon(J.id("apiCode")); 
  Jet.showWait(JSON.stringify(p));
  p.top-=J.id("apiCode").scroll();
  p.left-=J.id("apiCode").scrollX();
  J.id("cursor").css({
    top:(p.top)+"px",
    left:(p.left-3)+"px"
  });
...
function geneViewCode(){
  ...
  var html=J.id("apiCode").val().replaceAll("<","&lt;").replaceAll(">","&gt;");
  html=geneSign(html);
  html=geneKey(html);
